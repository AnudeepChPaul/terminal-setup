...
return {
  ...
  keys = {
    ...
    { "<C-n>h",     ":nohl<CR>",                                   desc = "Clear search highlights" },
    ...
    { "L",          'V"-y"-p',                                     desc = "Duplicates a line without copying to clipboard" },
    { "<space>li",  ":IBLToggle<CR>",                              desc = "Toggles indentation guides" },
    { "<space>lw",  ":set list!<CR>",                              desc = "Toggles list characters like Tab, space, newline" },
    { "<space>lr",  ":set rnu!<CR>",                               desc = "Toggles relative numbering" },
    { "x",          '"_x',                                         desc = "Delete single character without copying into register" },
    { "U",          "<C-r>",                                       desc = "Redo" },
    ...
    { "J",          "mzJ`z",                                       desc = "Bring botton line at the current line end" },
    ...
    { "n",          "nzzzv",                                       desc = "Goto next seach occurrence with cursor at center" },
    { "N",          "Nzzzv",                                       desc = "Goto previous seach occurrence with cursor at center" },
    ...
    { "J",          ":m '>+1<CR>gv=gv",                            desc = "Moves line one down",                                      mode = { "v", "x" } },
    { "K",          ":m '<-2<CR>gv=gv",                            desc = "Moves line one up",                                        mode = { "v", "x" } },
    { "<",          "<gv",                                         desc = "Indents left",                                             mode = { "v", "x" } },
    { ">",          ">gv",                                         desc = "Indents right",                                            mode = { "v", "x" } },
    { "p",          '"_dP"',                                       desc = "Paste's a line without copying the replacement",           mode = { "v", "x" } },
    ...
    { "me",         _K.goto_next_diagnostic,                       desc = "Goto next diagnostic (including error, info, warning etc)" },
    ...
    { "mcs",        "<Plug>CaserVSnakeCase",                       desc = "Change case to snake_case",                                mode = { "n", "v", "x" } },
    ...
    -- { "<c-;>",      "<Plug>(copilot-next)",                        desc = "Copilot next suggestion",                                  mode = "i" },
    -- { "<c-,>",      "<Plug>(copilot-previours)",                   desc = "Copilot previous suggestion",                              mode = "i" },
    -- { "<c-space>",  "<Plug>(copilot-suggest)",                     desc = "Copilot suggest",                                          mode = "i" },
    { "<C-\\>",     "<ESC><ESC>",                                  desc = "Escape",                                                   mode = "i" },
    { "<C-c>",      "<ESC><ESC><ESC>",                             desc = "Escape",                                                   mode = "i" },
    { "<c-h>",      "<left>",                                      desc = "Insert mode (Move left)",                                  mode = "i" },
    { "<c-l>",      "<right>",                                     desc = "Insert mode (Move right)",                                 mode = "i" },
    { "<c-j>",      "<down>",                                      desc = "Insert mode (Move down)",                                  mode = "i" },
    { "<c-k>",      "<up>",                                        desc = "Insert mode (Move up)",                                    mode = "i" },
    {
      "<space>lf",
      function()
  ...
  init = function()
    ...
    if tree_exists then
      ...
      wk.add({
        { "<c-e>",     ":Neotree position=float <cr>",                              desc = "Toggle file explorer" },
        { "<space>fl", ":Neotree position=float reveal=true reveal_force_cwd <cr>", desc = "Toggle file explorer" },
      }, { mode = "n" });
    end
  end,
}
